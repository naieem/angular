...
if (!function_exists('portfolio_list')) {
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if ($type == 'masonry') {
            ...
            $html .= "</div>";
            $page = get_page_by_title( 'Our Work' );
            if (get_next_posts_link()) {
                if ($show_load_more == "yes" || $show_load_more == "") {
                    $html .= '<div class="custom_portfolio_paging"><span class="custom_load_more"><a href="'.get_page_link($page->ID).'">Show More</a></span></div>';
                    $html .= '<div class="custom_portfolio_paging_loading"><a href="javascript: void(0)" class="qbutton">'.__('Loading...', 'qode').'</a></div>';
                }
            }
            $html .= "</div>";
            wp_reset_query();
